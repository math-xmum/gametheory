...
lemma Sum_Eq_zero_iff_all_zero' {α : Type*} [Fintype α]  {f : α → ℝ}
  (H : ∀ i, 0 ≤ f i) :
  ∑ i, f i = 0 ↔ ∀ i, f i = 0 := by
  have H : ∀ i∈ Finset.univ, 0 ≤ f i := by intro i _; exact H i
  constructor
  . intro h
    have := (Sum_Eq_zero_iff_all_zero (s := Finset.univ) (H)).1 h
