...
@[simp]
def maxb : ℝ  := Finset.sup' Finset.univ (⟨Nontrivial a.I,  (Finset.mem_univ _)⟩ ) sorry

/-- There exists a participant `i` whose bid equals the highest bid. -/
lemma exists_max : ∃ i: a.I, b i = a.maxb b := by
   obtain ⟨i,  _, h2⟩ := Finset.exists_mem_eq_sup' (⟨Nontrivial a.I, (Finset.mem_univ _)⟩ ) b
   exact ⟨i, symm h2⟩
